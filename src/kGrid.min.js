...
}else{g=g/2}if(g>0){$(f).css("padding-top",g)}})}$(b.div).data("datos",c);$(b.div).data("totalDatos",b.totalDatos);$(b.div).data("pagina",b.pagina);$(b.div).data("totalPaginas",b.totalPaginas);$("#kGrid_"+b.div.id+"_pagina").val(b.pagina).data("pagina",b.pagina);$("#kGrid_"+b.div.id+"_totalPaginas").html(b.totalPaginas);$("#kGrid_"+b.div.id+"_primera_pagina").attr("disabled",b.pagina==1);$("#kGrid_"+b.div.id+"_pagina_anterior").attr("disabled",b.pagina==1);$("#kGrid_"+b.div.id+"_ultima_pagina").attr("disabled",b.pagina==b.totalPaginas);$("#kGrid_"+b.div.id+"_siguiente_pagina").attr("disabled",b.pagina==b.totalPaginas)}else{if(e.mensaje){b.nuevo_mensaje(e.tipoMensaje,e.mensaje)}}if(typeof b.load_complete=="function"){b.load_complete.call(this,e)}},async:false})},cargar_entrada:function(t){var i=this;var m=t==undefined;var s="kGrid_"+i.div.id+"_"+(m?("nuevo_"+i.nuevos):t[i.id]);if(m){if($("#"+s).is(":visible")){var n=$("#"+s).find("input[data-editando]").first();if(n.length){n.focus()}else{i.nuevos++;i.cargar_entrada(t)}return}if(!i.tarjetas&&i.permisos.guardar){t={}}else{console.error('Para agregar entradas vacías la grilla no debe ser tipo tarjeta y debe configurar el permiso "guardar"');return}}var l=i.tarjetas?function(x,y){x.html(y)}:function(x,y){x.val(y)};var e=$("<div>").attr("id",s).attr("data-pk",t[i.id]).addClass("form-group"+(i.tarjetas?" well":""));var b=m?true:false;if(!m&&typeof i.estado=="function"){b=i.estado.call(this,t)}if(i.onclick){var w=typeof i.onclick=="function"?i.onclick:function(){window.open(i.onclick,"_self")};e.addClass("kbtn").click(function(){w.call(this,t)})}else{if(i.ondblclick){if((typeof i.ondblclick=="function")||(b&&typeof i.permisos.editar=="function")){var j=typeof i.ondblclick=="function"?i.ondblclick:function(){i.permisos.editar.call(this,t)};e.dblclick(function(){j.call(this,t)})}}}var k=$(i.permisos.guardar?"<form>":"<div>").addClass("col-md-"+(i.tarjetas?"7":"11")).appendTo(e);var v=$("<div>").addClass("text-right col-md-"+(i.tarjetas?"5":"1")).appendTo(e);var p=$("<div>").addClass("pull-right").addClass("kacciones").appendTo(v);var r=$("<div>").addClass("kscores pull-right").appendTo(v);$.each(i.campos,function(E,z){var x;if(z.tipo!="score"){var y=z.ancho;var C="<div>";if(i.tarjetas){y=6;C="<p>";if(z.tipo=="destacado"||z.tipo=="encabezado"){y=12}if(z.tipo=="encabezado"){C="<h2>"}}x=$(C).addClass("col-md-"+y).appendTo(k)}else{x=$("<p>").appendTo($("<div>").addClass("text-center pull-left kscore").appendTo(r))}if(!i.tarjetas){var B=$("<label>").addClass("klabel").addClass("visible-xs visible-sm").html(i.etiquetas[E]).appendTo(x)}var A=i.tarjetas?x:$("<input>").addClass("form-control").attr("readonly",true).attr("name",z.nombre).appendTo(x);l(A,typeof z.formato=="function"?z.formato.call(this,t[z.nombre],t):t[z.nombre]);if(i.tarjetas&&z.titulo&&z.titulo!=""){if(z.tipo!="score"){$("<label>").addClass("text-muted").html(z.titulo+"&nbsp; &nbsp;").prependTo(x)}else{$("<br>").appendTo(x);$("<small>").html(z.titulo).appendTo(x)}}if(z.atributos!=undefined){$.each(z.atributos,function(F,G){A.attr(F,G)});var D=["disabled","readonly"];$.each(D,function(F,G){if(z.atributos[G]=="false"){A.removeAttr(G)}A.attr("data-"+G,z.atributos[G])});if(!i.tarjetas&&z.atributos.type=="checkbox"){if(A.val()=="true"){A.attr("checked","checked")}A.removeClass("form-control").attr("data-pk",t[i.id]).dblclick(function(F){F.stopPropagation()});if(A.attr("readonly")){A.attr("disabled","disabled")}A.parent().addClass("text-center")}}});var u=i.tarjetas?"fa-3x":"fa-lg";var o=function(B,y,A,z){var x=$("<a>").attr("id",s+"_"+B).addClass("text-muted kaccion").attr("title",y).attr("href","JavaScript:void(0);").html('<i class="fa '+u+" fa-"+A+'"></i>').hover(function(){$(this).removeClass("text-muted").addClass("text-"+z)},function(){$(this).addClass("text-muted").removeClass("text-"+z)});return x};var h=function(){$("#"+s+" form").find("input[readonly]").each(function(y,z){if(($(z).attr("data-disabled")!="true"&&$(z).attr("data-readonly")!="true")||(m&&$(z).attr("data-creable"))){$(z).removeAttr("readonly").removeAttr("disabled").attr("data-editando",true)}});$("#"+s+"_editar").hide();if(!m){$("#"+s+"_remover").hide()}$("#"+s+"_guardar").fadeIn();$("#"+s).find("input[data-editando]").first().focus()};if(b){if(i.permisos.editar){var d=o("editar","Editar","pencil","primary");if(typeof i.permisos.editar=="function"){d.click(function(x){x.stopPropagation();i.permisos.editar.call(this,t)})}else{if(i.permisos.guardar){var f=o("guardar","Guardar","save","primary");f.hide();var g=typeof i.permisos.guardar=="function"?function(x){i.permisos.guardar.call(this,x)}:function(x){$.ajax({type:"POST",url:i.permisos.guardar,data:x,success:function(y){i.cargar()}})};f.click(function(x){x.stopPropagation();$("#"+s+" form").find("input[data-editando]").each(function(y,z){$(z).attr("readonly",$(z).attr("data-readonly")!="false").removeAttr("data-editando");if($(z).attr("type")=="checkbox"){$(z).attr("disabled",$(z).attr("data-disabled")!="false")
}});$("#"+s+"_guardar").hide();$("#"+s+"_editar").fadeIn();$("#"+s+"_remover").fadeIn();g($.extend(t,$("#"+s+" form").serialize()))}).appendTo(p);d.click(function(x){x.stopPropagation();h()})}}if(!m){d.appendTo(p)}}if(i.permisos.remover){var q=o("remover","Remover","times","danger");if(!m&&typeof i.permisos.remover=="function"){q.click(function(x){x.stopPropagation();i.permisos.remover.call(this,t)})}else{q.click(function(x){x.stopPropagation();$("#"+s).remove()})}q.appendTo(p)}}else{if(typeof i.permisos.activar=="function"){e.addClass("has-error");var c=o("reactivar","Reactivar","check","success");c.click(function(x){x.stopPropagation();i.permisos.activar.call(this,t)}).appendTo(p)}}$.each(i.botones,function(x,y){if(typeof y.mostrar!="function"||y.mostrar.call(this,t)){var z=$("<a>").attr("title",y.comentario).addClass("text-muted kaccion").attr("href",(y.enlace!=undefined)?y.enlace:"Javascript:void(0);").html('<i class="fa '+u+" "+y.icono+'"></i>').hover(function(){$(this).removeClass("text-muted")},function(){$(this).addClass("text-muted")});if(y.onclick!=undefined){z.click(function(A){A.stopPropagation();y.onclick.call(this,t)})}if(y.atributos!=undefined){$.each(y.atributos,function(A,B){z.attr(A,B)})}z.appendTo(p)}});if(m){e.prependTo(i.grilla);h()}else{e.appendTo(i.grilla)}if(!i.tarjetas){e.after($("<hr>").addClass("visible-xs visible-sm"))}},cargar_paginador:function(){var d=this;if(!$(d.paginador).length){return}var e="kGrid_"+d.div.id+"_";var c=$("<div>").attr("id",e+"paginador").addClass("kGrid-paginador btn-group").appendTo(d.paginador);$("<button>").attr("id",e+"primera_pagina").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-step-backward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","primera")});$("<button>").attr("id",e+"pagina_anterior").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-backward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","anterior")});var f=$("<div>").addClass("btn btn-default kpagina").css("height",$("#"+e+"pagina_anterior").outerHeight()).appendTo(c);$("<label>").html("Página ").appendTo(f);$("<input>").attr("id",e+"pagina").attr("type","text").addClass("pagina").css("width",$("#"+e+"pagina_anterior").outerWidth()).appendTo(f).keyup(function(h){if(h.keyCode==13){h.preventDefault();var g=parseInt($("#"+e+"pagina").val());if(isNaN(g)||g<0||g>d.totalPaginas){$("#"+e+"pagina").val($("#"+e+"pagina").data("pagina"));return}if(g!=$("#"+e+"pagina").data("pagina")){$("#"+d.div.id).kGrid("pagina",g)}}});var b=$("<label>").html(" de ").appendTo(f);$("<span>").attr("id",e+"totalPaginas").appendTo(b);$("<button>").attr("id",e+"siguiente_pagina").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-forward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","siguiente")});$("<button>").attr("id",e+"ultima_pagina").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-step-forward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","ultima")})},cargar_estilos:function(){if($("#kgrid_estilos").length){return}var e={".kgrid .kbtn":["cursor: pointer"],".kgrid .kbtn:hover":["background: #ECECF0","border: 1px solid #cacaca"],".kgrid h2":["padding-bottom: 10px"],".kgrid .kscore":["width: 0","height: 0","border: 1px solid #999999","background: #ECECF0","font-size: 1.7em","margin-left: 10px","overflow: hidden","border-radius: 50%"],".kgrid .kscore p":["margin-top: 25%"],".kgrid .kscore small":["font-size: 0.5em"],".kgrid .klabel":["margin-top: 20px"],".kgrid .kaccion":["margin-left: 15px"],".kgrid .kpagina":["overflow: hidden","padding-top: 2px"],".kgrid .kpagina input":["margin: 0 5px","text-align: center"]};var b="";$.each(e,function(f,g){b+=f+"{";$.each(g,function(h,i){b+=i+";"});b+="}"});var d=$("head").find("link,style").first();var c=$("<style>").attr("id","kgrid_estilos").html(b);if(d.length){d.before(c)}else{c.prependTo("head")}},cambiar_seleccion:function(d,c){var b=this;b.seleccionados[d]=c;b.refrescar_seleccionados()},seleccionar:function(c){var b=this;b.seleccionados={};$.each(c,function(e,d){b.seleccionados[d]=true});$("."+b.div.id+"_seleccionar_row").each(function(d,e){$(e).removeAttr("checked");if(b.seleccionados[$(e).data("pk")]){$(e).prop("checked",true)}});b.refrescar_seleccionados()},refrescar_seleccionados:function(){var b=this;var c=[];$.each(b.seleccionados,function(e,d){if(d){c.push(e)}});$(b.div).data("seleccionados",c)},agregar:function(c){var b=this;b.cargar_entrada(c)}}})();
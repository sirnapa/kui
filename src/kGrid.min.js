...
}else{g=g/2}if(g>0){$(f).css("padding-top",g)}})}$(b.div).data("datos",c);$(b.div).data("totalDatos",b.totalDatos);$(b.div).data("pagina",b.pagina);$(b.div).data("totalPaginas",b.totalPaginas);$("#kGrid_"+b.div.id+"_pagina").val(b.pagina).data("pagina",b.pagina);$("#kGrid_"+b.div.id+"_totalPaginas").html(b.totalPaginas);$("#kGrid_"+b.div.id+"_primera_pagina").attr("disabled",b.pagina==1);$("#kGrid_"+b.div.id+"_pagina_anterior").attr("disabled",b.pagina==1);$("#kGrid_"+b.div.id+"_ultima_pagina").attr("disabled",b.pagina==b.totalPaginas);$("#kGrid_"+b.div.id+"_siguiente_pagina").attr("disabled",b.pagina==b.totalPaginas)}else{if(e.mensaje){b.nuevo_mensaje(e.tipoMensaje,e.mensaje)}}if(typeof b.load_complete=="function"){b.load_complete.call(this,e)}},async:false})},cargar_entrada:function(v){var i=this;var m=v==undefined;var u="kGrid_"+i.div.id+"_"+(m?("nuevo_"+i.nuevos):v[i.id]);if(m){if($("#"+u).is(":visible")){var n=$("#"+u).find("input[data-editando]").first();if(n.length){n.focus()}else{i.nuevos++;i.cargar_entrada(v)}return}if(!i.tarjetas&&i.permisos.guardar){v={}}else{console.error('Para agregar entradas vacías la grilla no debe ser tipo tarjeta y debe configurar el permiso "guardar"');return}}var l=i.tarjetas?function(A,B){A.html(B)}:function(A,B){A.val(B)};var e=$("<div>").attr("id",u).attr("data-pk",v[i.id]).addClass("form-group"+(i.tarjetas?" well":""));var b=m?true:false;if(!m&&typeof i.estado=="function"){b=i.estado.call(this,v)}if(i.onclick){var z=typeof i.onclick=="function"?i.onclick:function(){window.open(i.onclick,"_self")};e.addClass("kbtn").click(function(){z.call(this,v)})}else{if(i.ondblclick){if((typeof i.ondblclick=="function")||(b&&typeof i.permisos.editar=="function")){var j=typeof i.ondblclick=="function"?i.ondblclick:function(){i.permisos.editar.call(this,v)};e.dblclick(function(){j.call(this,v)})}}}var k=$(i.permisos.guardar?"<form>":"<div>").addClass("col-md-"+(i.tarjetas?"7":"11")).appendTo(e);var x=$("<div>").addClass("text-right col-md-"+(i.tarjetas?"5":"1")).appendTo(e);var q=$("<div>").addClass("pull-right").addClass("kacciones").appendTo(x);var s=$("<div>").addClass("kscores pull-right").appendTo(x);$.each(i.campos,function(H,C){var A;if(C.tipo!="score"){var B=C.ancho;var F="<div>";if(i.tarjetas){B=6;F="<p>";if(C.tipo=="destacado"||C.tipo=="encabezado"){B=12}if(C.tipo=="encabezado"){F="<h2>"}}A=$(F).addClass("col-md-"+B).appendTo(k)}else{A=$("<p>").appendTo($("<div>").addClass("text-center pull-left kscore").appendTo(s))}if(!i.tarjetas){var E=$("<label>").addClass("klabel").addClass("visible-xs visible-sm").html(i.etiquetas[H]).appendTo(A)}var D=i.tarjetas?A:$("<input>").addClass("form-control").attr("readonly",true).attr("name",C.nombre).appendTo(A);if(typeof C.formato=="function"){v[C.nombre]=C.formato.call(this,v[C.nombre],v)}l(D,v[C.nombre]);if(i.tarjetas&&C.titulo&&C.titulo!=""){if(C.tipo!="score"){$("<label>").addClass("text-muted").html(C.titulo+"&nbsp; &nbsp;").prependTo(A)}else{$("<br>").appendTo(A);$("<small>").html(C.titulo).appendTo(A)}}if(C.atributos!=undefined){$.each(C.atributos,function(I,J){D.attr(I,J)});var G=["disabled","readonly"];$.each(G,function(I,J){if(C.atributos[J]=="false"){D.removeAttr(J)}D.attr("data-"+J,C.atributos[J])});if(!i.tarjetas&&C.atributos.type=="checkbox"){if(D.val()=="true"){D.attr("checked","checked")}D.removeClass("form-control").attr("data-pk",v[i.id]).dblclick(function(I){I.stopPropagation()});if(D.attr("readonly")){D.attr("disabled","disabled")}D.parent().addClass("text-center")}}});var w=i.tarjetas?"fa-3x":"fa-lg";var o=function(E,B,D,C){var A=$("<a>").attr("id",u+"_"+E).addClass("text-muted kaccion").attr("title",B).attr("href","JavaScript:void(0);").html('<i class="fa '+w+" fa-"+D+'"></i>').hover(function(){$(this).removeClass("text-muted").addClass("text-"+C)},function(){$(this).addClass("text-muted").removeClass("text-"+C)});return A};var h=function(){$("#"+u+" form").find("input[readonly]").each(function(A,B){if(($(B).attr("data-disabled")!="true"&&$(B).attr("data-readonly")!="true")||(m&&$(B).attr("data-creable"))){$(B).removeAttr("readonly").removeAttr("disabled").attr("data-editando",true).keyup(function(C){if(C.keyCode==13){C.preventDefault();$("#"+u+" form").submit()}})}});$("#"+u+"_editar").hide();if(!m){$("#"+u+"_remover").hide();$("#"+u+"_deshacer").fadeIn()}$("#"+u+"_guardar").fadeIn();$("#"+u).find("input[data-editando]").first().focus()};var g=function(){$("#"+u+" form").find("input[data-editando]").each(function(A,B){$(B).attr("readonly",$(B).attr("data-readonly")!="false").removeAttr("data-editando").unbind("keyup");if($(B).attr("type")=="checkbox"){$(B).attr("disabled",$(B).attr("data-disabled")!="false")}});$("#"+u+"_guardar").hide();$("#"+u+"_deshacer").hide();$("#"+u+"_editar").fadeIn();$("#"+u+"_remover").fadeIn()};var t=function(){var A=$(i.div).data("datos")[$("#"+u).attr("data-pk")];var B=$("#"+u+" form").find("input[data-editando]");g();B.each(function(C,D){if($(D).attr("type")=="checkbox"){$(D).prop("checked",A[$(D).attr("name")])
}else{$(D).val(A[$(D).attr("name")])}})};if(b){if(i.permisos.editar){var d=o("editar","Editar","pencil","primary");if(typeof i.permisos.editar=="function"){d.click(function(A){A.stopPropagation();i.permisos.editar.call(this,v)})}else{if(i.permisos.guardar){var f=o("guardar","Guardar","save","primary");f.hide();var y=typeof i.permisos.guardar=="function"?function(A){i.permisos.guardar.call(this,A)}:function(A){$.ajax({type:"POST",url:i.permisos.guardar,data:A,success:function(B){i.cargar()}})};$.kUI.validaciones.cargar();$(k).validate({showErrors:function(A,B){$.kUI.validaciones.error(this,A,B)},submitHandler:function(A){$.kUI.validaciones.fechas(A);g();$.each($("#"+u+" form").serializeArray(),function(B,C){v[C.name]=C.value});y(v);return false}});f.click(function(A){A.stopPropagation();$("#"+u+" form").submit()}).appendTo(q);var p=o("deshacer","Deshacer cambios","undo","danger");p.hide().click(function(A){A.stopPropagation();t()}).appendTo(q);d.click(function(A){A.stopPropagation();h()})}}if(!m){d.appendTo(q)}}if(i.permisos.remover){var r=o("remover","Remover","times","danger");if(!m&&typeof i.permisos.remover=="function"){r.click(function(A){A.stopPropagation();i.permisos.remover.call(this,v)})}else{r.click(function(A){A.stopPropagation();$("#"+u).remove()})}r.appendTo(q)}}else{if(typeof i.permisos.activar=="function"){e.addClass("has-error");var c=o("reactivar","Reactivar","check","success");c.click(function(A){A.stopPropagation();i.permisos.activar.call(this,v)}).appendTo(q)}}$.each(i.botones,function(A,B){if(typeof B.mostrar!="function"||B.mostrar.call(this,v)){var C=$("<a>").attr("title",B.comentario).addClass("text-muted kaccion").attr("href",(B.enlace!=undefined)?B.enlace:"Javascript:void(0);").html('<i class="fa '+w+" "+B.icono+'"></i>').hover(function(){$(this).removeClass("text-muted")},function(){$(this).addClass("text-muted")});if(B.onclick!=undefined){C.click(function(D){D.stopPropagation();B.onclick.call(this,v)})}if(B.atributos!=undefined){$.each(B.atributos,function(D,E){C.attr(D,E)})}C.appendTo(q)}});if(m){e.prependTo(i.grilla);h()}else{e.appendTo(i.grilla)}if(!i.tarjetas){e.after($("<hr>").addClass("visible-xs visible-sm"))}},cargar_paginador:function(){var d=this;if(!$(d.paginador).length){return}var e="kGrid_"+d.div.id+"_";var c=$("<div>").attr("id",e+"paginador").addClass("kGrid-paginador btn-group").appendTo(d.paginador);$("<button>").attr("id",e+"primera_pagina").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-step-backward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","primera")});$("<button>").attr("id",e+"pagina_anterior").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-backward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","anterior")});var f=$("<div>").addClass("btn btn-default kpagina").css("height",$("#"+e+"pagina_anterior").outerHeight()).appendTo(c);$("<label>").html("Página ").appendTo(f);$("<input>").attr("id",e+"pagina").attr("type","text").addClass("pagina").css("width",$("#"+e+"pagina_anterior").outerWidth()).appendTo(f).keyup(function(h){if(h.keyCode==13){h.preventDefault();var g=parseInt($("#"+e+"pagina").val());if(isNaN(g)||g<0||g>d.totalPaginas){$("#"+e+"pagina").val($("#"+e+"pagina").data("pagina"));return}if(g!=$("#"+e+"pagina").data("pagina")){$("#"+d.div.id).kGrid("pagina",g)}}});var b=$("<label>").html(" de ").appendTo(f);$("<span>").attr("id",e+"totalPaginas").appendTo(b);$("<button>").attr("id",e+"siguiente_pagina").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-forward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","siguiente")});$("<button>").attr("id",e+"ultima_pagina").attr("type","button").addClass("btn btn-default").html($("<i>").addClass("fa fa-step-forward")).appendTo(c).click(function(){$("#"+d.div.id).kGrid("pagina","ultima")})},cargar_estilos:function(){if($("#kgrid_estilos").length){return}var e={".kgrid .kbtn":["cursor: pointer"],".kgrid .kbtn:hover":["background: #ECECF0","border: 1px solid #cacaca"],".kgrid h2":["padding-bottom: 10px"],".kgrid .kscore":["width: 0","height: 0","border: 1px solid #999999","background: #ECECF0","font-size: 1.7em","margin-left: 10px","overflow: hidden","border-radius: 50%"],".kgrid .kscore p":["margin-top: 25%"],".kgrid .kscore small":["font-size: 0.5em"],".kgrid .klabel":["margin-top: 20px"],".kgrid .kaccion":["margin-left: 15px"],".kgrid .kpagina":["overflow: hidden","padding-top: 2px"],".kgrid .kpagina input":["margin: 0 5px","text-align: center"]};var b="";$.each(e,function(f,g){b+=f+"{";$.each(g,function(h,i){b+=i+";"});b+="}"});var d=$("head").find("link,style").first();var c=$("<style>").attr("id","kgrid_estilos").html(b);if(d.length){d.before(c)}else{c.prependTo("head")}},cambiar_seleccion:function(d,c){var b=this;b.seleccionados[d]=c;b.refrescar_seleccionados()},seleccionar:function(c){var b=this;b.seleccionados={};$.each(c,function(e,d){b.seleccionados[d]=true});
$("."+b.div.id+"_seleccionar_row").each(function(d,e){$(e).removeAttr("checked");if(b.seleccionados[$(e).data("pk")]){$(e).prop("checked",true)}});b.refrescar_seleccionados()},refrescar_seleccionados:function(){var b=this;var c=[];$.each(b.seleccionados,function(e,d){if(d){c.push(e)}});$(b.div).data("seleccionados",c)},agregar:function(c){var b=this;b.cargar_entrada(c)}}})();